...
> ⚠️ **불변 정책**: 이 문서의 규칙은 절대 변경되지 않습니다. 특히 "메인 반영 후 수정은 반드시 스테이징에서" 규칙은 철저히 준수해야 합니다.
...
### 🔴 절대 규칙: 메인 반영 후 수정은 반드시 스테이징에서

1. **절대 main 브랜치에 직접 커밋하지 않기** ⚠️ 불변 정책
   - 모든 변경사항은 staging에서 테스트 후 main으로 머지
   - **메인 반영 후 다시 고쳐지는 것도 반드시 staging에서 작업**
   - **메인에서 직접 수정 금지** (긴급 핫픽스 제외)
   - **이 규칙은 절대 변경되지 않습니다**
...
```
1. 스테이징에서 개발 → v1.5.XX-staging
2. 메인으로 머지 → v1.5.XX (접미사 제거)
3. 또 수정 필요? → 다시 스테이징으로! (v1.5.XX+1-staging)
4. 반복...
```

**⚠️ 중요: 메인 반영 후 수정은 반드시 스테이징에서 작업합니다. 이 규칙은 변경되지 않습니다.**
