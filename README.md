...
- **`main`**: 프로덕션 환경 (고객 사용)
- **`staging`**: 스테이징 환경 (개발 및 테스트)

자세한 내용은 [브랜치 전략 가이드](docs/BRANCH_STRATEGY.md)를 참조하세요.
...
### v1.5.0 (2025-11-08)
- 슈퍼어드민 대시보드에 `전체 현황 ↔︎ 선택된 가게` 우주선 토글 탑재
- 가게별 이벤트 메트릭 도감과 즉석 검색 기능 추가
- 모달 기반 계정 관리 UX로 운영 중지/삭제 절차 간소화
